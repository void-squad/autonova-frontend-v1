...
export default function ProjectsListPage() {
  ...
  const [total, setTotal] = useState(() =>
    computeProjectList(storeProjects, { page: 1, pageSize: PAGE_SIZE }).total,
  );
  ...
  const columns = useMemo<DataTableColumn<Project>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (project) => (
          <Button size="sm" variant="outline" onClick={() => navigate(`/employee/projects/${project.id}`)}>
            View
          </Button>
        ),
      },
      ...
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="text-muted-foreground">Track modification requests, quotes, and progress in one place.</p>
          </div>
          <Button onClick={() => navigate("/employee/projects/new")}>New project</Button>
        </div>
